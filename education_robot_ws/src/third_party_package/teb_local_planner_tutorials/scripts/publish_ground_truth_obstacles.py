...
def callback_base_pose_ground_truth(base_pose_ground_truth, obst_id):
    ...
    obstacles_msg.obstacles[i].polygon.points[
        0
    ].x = base_pose_ground_truth.pose.pose.position.x
    obstacles_msg.obstacles[i].polygon.points[
        0
    ].y = base_pose_ground_truth.pose.pose.position.y
    obstacles_msg.obstacles[i].polygon.points[
        0
    ].z = base_pose_ground_truth.pose.pose.position.z

    # ORIENTATIONS
    yaw = math.atan2(
        base_pose_ground_truth.twist.twist.linear.y,
        base_pose_ground_truth.twist.twist.linear.x,
    )
...
def init_subscribers():
    ...
    for topic in bpgt_topics:
        ...
        sub = rospy.Subscriber(
            topic, Odometry, callback_base_pose_ground_truth, obst_id
        )
