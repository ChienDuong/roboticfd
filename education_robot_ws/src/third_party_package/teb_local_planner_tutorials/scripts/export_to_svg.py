...
SCALE_VELOCITY_VEC = (
    0.4  # Scaling of velocity vectors -> 1 cell = 1/SCALE_VELOCITY_VEC m/s
)
...
def quaternion2YawDegree(orientation):
    ...
    yawRad = math.atan2(
        ...
        1 - 2 * (pow(orientation.y, 2) + pow(orientation.z, 2)),
    )
...
if __name__ == "__main__":
    ...
    svg.viewbox(
        GRID_X_MIN * SCALE - 1 * SCALE,
        GRID_Y_MIN * SCALE - 1 * SCALE,
        GRID_X_MAX * SCALE - GRID_X_MIN * SCALE + 2 * SCALE,
        GRID_Y_MAX * SCALE - GRID_Y_MIN * SCALE + 2 * SCALE,
    )
    ...
    for y in range(GRID_Y_MAX):
        hLines.add(
            svg.line(
                start=(GRID_X_MIN * SCALE, SCALE + y * SCALE),
                end=(GRID_X_MAX * SCALE, SCALE + y * SCALE),
            )
        )
    ...
    for x in range(GRID_X_MAX):
        vLines.add(
            svg.line(
                start=(SCALE + x * SCALE, GRID_Y_MIN * SCALE),
                end=(SCALE + x * SCALE, GRID_Y_MAX * SCALE),
            )
        )
    ...
    legend.translate(
        tx=GRID_X_MIN * SCALE, ty=GRID_Y_MAX * SCALE + 30
    )  # Move legend to buttom left corner
    svg.add(legend)
    ...
    for index, traj in enumerate(feedbackMsg.trajectories):
        ...
        for point in traj.trajectory:
            points.append(
                (point.pose.position.x * SCALE, -point.pose.position.y * SCALE)
            )  # y is negative in image coordinates
            # svgwrite rotates clockwise!

        if (
            index == feedbackMsg.selected_trajectory_idx
        ):  # highlight currently selected teb
            line = svg.add(
                svg.polyline(
                    points=points,
                    fill="none",
    ...
    goal_pose = (
        feedbackMsg.trajectories[0]
        .trajectory[len(feedbackMsg.trajectories[0].trajectory) - 1]
        .pose
    )
    ...
    svg.add(
        svg.circle(
            center=(start_position.x * SCALE, -start_position.y * SCALE),
            r=10,
            stroke_width=1,
            stroke="blue",
            fill="blue",
    ...
    svg.add(
        svg.text(
            "Start",
            (start_position.x * SCALE - 70, -start_position.y * SCALE + 45),
            font_size=35,
        )
    )  # Add label
    svg.add(
        svg.circle(
            center=(goal_position.x * SCALE, -goal_position.y * SCALE),
            r=10,
            stroke_width=1,
            stroke="red",
            fill="red",
    ...
    svg.add(
        svg.text(
            "Goal",
            (goal_position.x * SCALE - 40, -goal_position.y * SCALE + 45),
            font_size=35,
        )
    )  # Add label

    # draw start arrow
    start_arrow = svg.polyline(
        [(0, -1), (6, -1), (5, -5), (15, 0), (5, 5), (6, 1), (0, 1)],
        fill="blue",
        opacity=1.0,
    )
    ...
    for obstacle in feedbackMsg.obstacles:
        if len(obstacle.polygon.points) == 1:  # point obstacle
            ...
            svg.add(
                svg.circle(
                    center=(point.x * SCALE, -point.y * SCALE),
                    r=OBSTACLE_DIST,
                    stroke_width=1,
                    stroke="grey",
                    fill="grey",
            ...
            svg.add(
                svg.text(
                    "Obstacle",
                    (point.x * SCALE - 70, -point.y * SCALE + 45),
                    font_size=35,
                )
            )  # Add label
        if len(obstacle.polygon.points) == 2:  # line obstacle
            ...
            svg.add(
                svg.line(
                    start=(line_start.x * SCALE, -line_start.y * SCALE),
                    end=(line_end.x * SCALE, -line_end.y * SCALE),
                    stroke="black",
                    fill="gray",
                    stroke_width=1,
                    opacity=1.0,
            ...
            svg.add(
                svg.text(
                    "Obstacle",
                    (line_start.x * SCALE - 70, -line_start.y * SCALE + 45),
                    font_size=35,
                )
            )  # Add label
        if len(obstacle.polygon.points) > 2:  # polygon obstacle
            ...
            svg.add(
                svg.text(
                    "Obstacle",
                    (
                        obstacle.polygon.points[0].x * SCALE - 70,
                        -obstacle.polygon.points.y * SCALE + 45,
                    ),
                    font_size=35,
                )
            )  # Add label
