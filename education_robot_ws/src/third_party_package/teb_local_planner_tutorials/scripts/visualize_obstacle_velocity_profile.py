...
def twist_callback(data):
    ...
    if start_t == 0:
        start_t = (
            rospy.Time.now()
        )  # cmd_vel doesn't provide any time information, therefore measure it
...
def velocity_plotter():
    ...
    rospy.loginfo(
        "Visualizing ground truth velocity profile published on '%s'.",
        topic_name_ground_truth_vel,
    )
    rospy.loginfo(
        "Visualizing estimated velocity profile published on '%s'.",
        topic_name_estimated_vel,
    )
