...
#include "edu_gazebo/edu_drive.h"
#include "math.h"

Turtlebot3Drive::Turtlebot3Drive()
  : nh_priv_("~")
{
  //Init gazebo ros edu node
...
void Turtlebot3Drive::updatecommandVelocity(double linear, double angular)
{
  ...
  if (abs(linear) > 0.2)
    if (cmd_vel.linear.x < 0)
      cmd_vel.linear.x  = -0.2 * 1000;
    else
      cmd_vel.linear.x  = 0.2 * 1000;
  else
    cmd_vel.linear.x  = linear * 1000;
  cmd_vel.angular.z = angular * 1000;
...
{
  static uint8_t edu_state_num = 0;

  switch(edu_state_num)
  {
    case GET_TB3_DIRECTION:
      if (scan_data_[CENTER] > check_forward_dist_)
      {
        if (scan_data_[LEFT] < check_side_dist_)
        {
          ...
          edu_state_num = TB3_RIGHT_TURN;
        }
        else if (scan_data_[RIGHT] < check_side_dist_)
        {
          prev_tb3_pose_ = tb3_pose_;
          edu_state_num = TB3_LEFT_TURN;
        }
        else
        {
          edu_state_num = TB3_DRIVE_FORWARD;
        }
      }
    ...
    case TB3_DRIVE_FORWARD:
      ...
      edu_state_num = GET_TB3_DIRECTION;
      break;
...
{
  ros::init(argc, argv, "edu_drive");
  Turtlebot3Drive edu_drive;

  ros::Rate loop_rate(125);

  while (ros::ok())
  {
    edu_drive.controlLoop();
    ros::spinOnce();
    loop_rate.sleep();
  }
